...
class SimpleOAuthProvider(OAuthAuthorizationServerProvider[AuthorizationCode, RefreshToken, AccessToken]):
    ...
    def _register_default_clients(self):
        ...
        default_client = OAuthClientInformationFull(
            ...
            redirect_uris=[
                "http://localhost:3030/callback",                    # For existing client
                "http://localhost:6274/oauth/callback/debug",        # For new client
                "http://localhost:3000/callback",                    # Generic port
                "http://localhost:8080/callback",                    # Generic port
            ],
        ...
        logger.info(f"Pre-registered client: {default_client.client_id}")
